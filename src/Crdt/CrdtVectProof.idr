...
public export
merge_vect_commutes : (xs, ys : (Vect n Nat)) ->
  mergeCrdt xs ys = mergeCrdt ys xs

merge_vect_commutes [] [] = Refl
merge_vect_commutes (x::xs) (y::ys) = 
  let rec = mergeCrdt_nat_commutes x y in
  let rec2 = merge_vect_commutes xs ys in
    rewrite rec in
    rewrite rec2 in Refl

public export
merge_vect_idempotent : (xs : (Vect n Nat)) -> 
  mergeCrdt xs xs = xs

merge_vect_idempotent [] = Refl
merge_vect_idempotent (x::xs) = 
  let rec = mergeCrdt_nat_idempotent x in
  let rec2 = merge_vect_idempotent xs in
    rewrite rec in
    rewrite rec2 in Refl

public export
merge_vect_identity : {n : Nat} -> (xs : (Vect n Nat)) ->
  mergeCrdt (Data.Vect.replicate n Z) xs = xs

merge_vect_identity [] = Refl
merge_vect_identity (x::xs) = 
  let rec = mergeCrdt_nat_identity x in
