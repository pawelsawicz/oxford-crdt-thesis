...
update r = let newState = updateAt r.replicaIndex (+1) r.state in
            { state := newState } r
...
merge r1 r2 =  let newState = mergeLocal r1.state r2.state in
                { state := newState} r1
...
testQuery : Test
testQuery = let test = query testReplica1 == 3 in
                MkTest "Query for Replica equals 3" test

testUpdate : Test
testUpdate = let test = (update testReplica1).state == [0,1,3] in
                MkTest "Update for Replica equals [0,1,3]" test

testMerge : Test
testMerge = let test = (merge testReplica1 testReplica2).state == [5,6,3] in
